...
void loop() {
  ...
  Gas(); /* Detectando presenca de gas */

  Temp_Umid(); /* Registrando temperatura/umidade */
...
void Gas(){
  ...
  gasString = String(sensorValue);
  gasString.toCharArray(gasChar, gasString.length()+1);

  MQTT.publish("smartcarpuc/gas", gasChar); // envio para App
...
void Temp_Umid() {
  ...
  umdString = String(humidity);
  
  tempString.toCharArray(tempChar, tempString.length()+1);
  umdString.toCharArray(umdChar, umdString.length()+1);

  /* Verificando e mostrando temperatura*/
  if(temperature > 25){
    Serial.println("\nALERT: High temperature: " + tempString);
    //digitalWrite(rgb_red, HIGH);
  } else {
    Serial.println("\nModerate temperature: " + tempString);
    //digitalWrite(rgb_red, LOW);
  }
  MQTT.publish("smartcarpuc/temperatura", tempChar); // envio para App

  /* Mostrando umidade */
  Serial.println("Umidade: " + umdString);
  MQTT.publish("smartcarpuc/umidade", umdChar); // envio para App
}
